...
if [ "${PHP_EXTENSIONS}" != "" ]; then
    apk --update add --no-cache --virtual .build-deps build-base autoconf g++ libtool make curl-dev gettext-dev linux-headers libc-dev zlib-dev
fi
...
if [[ -z "${EXTENSIONS##*,protobuf,*}" ]]; then
    ...
    if [[ "$?" = "1" ]]; then
        ...
        # 必须跟版本号，防止protobuf更新后提高PHP版本限制
        printf "\n" | pecl install protobuf-3.12.0
        # 方便编译
...
if [[ -z "${EXTENSIONS##*,grpc,*}" ]]; then
    ...
    if [[ "$?" = "1" ]]; then
        echo "---------- Install grpc ----------"
        # 必须跟版本号，防止grpc更新后提高PHP版本限制
        printf "\n" | pecl install grpc-1.40.0
